...
# Build the application with musl tag for Alpine compatibility
RUN CGO_ENABLED=1 go build -tags musl -ldflags="-w -s" -o go-base-ms cmd/server/main.go

# Runtime stage
FROM alpine:3.22

# Update and install runtime dependencies
RUN apk update && apk upgrade && apk add --no-cache ca-certificates tzdata librdkafka wget
